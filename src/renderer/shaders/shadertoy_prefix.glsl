...
// The buffers declared by `custom-shader-buffer`, in the order declared.
// These are bound to a 1x1 zero texture when no buffer is declared for
// them, so reading one that does not exist is defined but useless.
