...
/// loop will still only run when custom shaders are used. Unfocused surfaces
/// animate at a quarter of the focused rate, so a window full of splits
/// costs a little over one focused surface rather than one per split. Note
/// that this still uses more CPU per terminal surface and can become quite
/// expensive depending on the shader and your terminal usage.
///
/// This can be changed at runtime and will affect all open terminals.
